...
namespace TheOtherRoles.Modules 
{
    public class ModUpdateBehaviour : MonoBehaviour
    {
        ...
        private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
        {
            ...
            var announcement = $"<size=150%>一个新的<color=#FC0303>{(isSubmerged ? "潜艇地图" : "超多职业CE")}</color>更新 {(isSubmerged ? SubmergedUpdate.Tag : TORUpdate.Tag)}可用</size>\n{(isSubmerged ? SubmergedUpdate.Content : TORUpdate.Content)}";
        ...
        public IEnumerator CoUpdate()
        {
            ...
            button.SetActive(false);
            popup.TextAreaTMP.text = $"更新中 {updateName}\n请稍等...";
